...
При подключении платы сервер сразу отправляет ей текущее значение частоты. При потере связи плата автоматически переподключается.
...
MCU генерирует квадратурные сигналы через PIO RP2040. Частота и направление управляются удалённо: плата подключается к серверу по WebSocket и обновляет параметры генерации в реальном времени без прерывания сигналов на других каналах.
...
Сервер отправляет сообщение при подключении платы и после каждого `POST /frequency`.
